using Microsoft.AspNetCore.Http;
using Microsoft.AspNetCore.Mvc;
using Timely.Dtos;
using Timely.Interfaces.IServices;
using Timely.Models;

namespace Timely2_Api.Controllers
{
    [Route("api/[controller]")]
    [ApiController]
    public class AccountsController : ControllerBase
    {
        private readonly IAccountServices _accountService;
        public AccountsController(IAccountServices empService)
        {
            _accountService = empService;
        }
        [HttpGet]
        public ActionResult<IEnumerable<Employee>> GetAll()
        {
            try
            {
                IEnumerable<Employee> Emp = _accountService.GetAll();
                return Ok(Emp);
            }
            catch (Exception)
            {
                return Content(" error ");
            }
        }

        [HttpGet("{uid}")]
        public ActionResult<Employee> GetByUid(string uid)
        {
            try
            {
                if (string.IsNullOrWhiteSpace(uid))
                    return BadRequest("Uid is required.");

                var Emps = _accountService.GetByUid(uid);
                if (Emps == null)
                    return NotFound("لا توجد فئه بهذا الرقم"); // 404

                return Ok(Emps); // 200

            }
            catch (Exception ex)
            {
                var message = ex.Message.ToString();
                return BadRequest($"{message}حدث خطا غير متوقع");
            }
        }
        [HttpPost]
        public ActionResult Create(EmplDot emplDto)
        {
            try
            {
                if (!ModelState.IsValid)
                    return BadRequest(ModelState);
                var empl = new Employee();
                empl.Email = emplDto.Email;
                empl.Password = emplDto.Password;
                var isCreated = _accountService.Create(empl);
                if (isCreated)
                    //  return CreatedAtAction(nameof(GetByUid), new { uid = category.Uid }, category); // 201

                    return Ok("تم انشاء الفئه بنجاح"); // 200

                return BadRequest("فشل انشاء الفئه"); // 400

            }
            catch (Exception ex)
            {
                var message = ex.Message.ToString();
                return BadRequest($"{message}حدث خطا غير متوقع");
            }
        }



        [HttpPut("{uid}")]
        public IActionResult Update(string uid, [FromBody] AccountUpdateDtos employee)
        {
            if (string.IsNullOrWhiteSpace(uid))
                return BadRequest("Uid is required.");

            if (employee == null)
                return BadRequest("Category payload is required.");

            if (!ModelState.IsValid)
                return ValidationProblem(ModelState);

            var exists = _accountService.GetByUid(uid);
            if (exists == null)
                return NotFound(); // 404

            // نضمن التحديث على نفس الـUid

            var NewEmp = new Employee
            {
                Uid = employee.Uid,
                Email = employee.Email,
                Password = employee.Password,
            };
            _accountService.Update(uid, NewEmp);
            return Ok("تم تحديث الفئة بنجاح");
        }


        // DELETE: api/categories/{uid}
        [HttpDelete("{uid}")]
        public IActionResult Delete(string uid)
        {
            if (string.IsNullOrWhiteSpace(uid))
                return BadRequest("Uid is required.");

            var exists = _accountService.GetByUid(uid);
            if (exists == null)
                return NotFound(); // 404

            _accountService.DeleteByUid(uid);
            return Ok("تم حذف الفئة بنجاح"); // 204
        }


    }
}